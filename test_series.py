...
class Series:
    ...
    def __init__(self, a=lambda n: Fraction(), seq=None, infinity_power=5):
        self.a = a
...
class PartialSum:
    def __init__(self, finite_sum):
        def a(n):
            if n < 1: raise ValueError("Indexing starts from 1")
            if n == 1: return finite_sum(1)
            else: return finite_sum(n) - finite_sum(n-1)
        self.a = a
...
def test_partial_sum():
    ...
    for n in range(6, 10):
        ...
        assert part_sum.a(n) == Fraction(n * (n + 3),           4 * (n + 1) * (n + 2)) - Fraction((n-1) * ((n-1) + 3), 4 * ((n-1) + 1) * ((n-1) + 2))
        assert part_sum.a(n) != Fraction(n * (n + 3) - (n-1) * ((n-1) + 3), 4 * (n + 1) * (n + 2) - 4 * ((n-1) + 1) * ((n-1) + 2))
...
def test_term_paper_1_2():
    ...
    assert Series(lambda n: math.log(n)/math.sqrt(n**8+n), infinity_power=1) == Series(lambda n: math.log(n)/math.sqrt(n**8+n), infinity_power=3)
    print(3/43)
    #assert  == 20.20
